...
if env_id in ['hopper-medium-replay-v0']:
    import d4rl

if env_id in ['Hopper-v3', 'hopper-medium-replay-v0']:
    action_space_size = 3
...
mujoco_sampled_efficientzero_config = dict(
    exp_name=f'data_sez/{env_id[:-3]}_sampled_efficientzero_ns{num_simulations}_ne{n_episode}_upc{update_per_collect}_rer{reanalyze_ratio}_bs-{batch_size}_pelw{policy_entropy_loss_weight}_seed{seed}',
    env=dict(
        env_id=env_id,
        action_clip=True,
        continuous=True,
